...
public class EntityGapingVoid extends Entity {
    ...
    public static final EntitySelector sucklector = new EntitySelector() {
    ...
    public static final EntitySelector nomlector = new EntitySelector() {
    ...
    public void onUpdate() {
        ...
        if (age >= maxLifetime) {
            ...
            if (age == 0) {
                //this.worldObj.playSound(this.posX, this.posY, this.posZ, "Avaritia:gapingVoid", 8.0F, 1.0F);
            }
            this.setAge(age+1);
        }
        ...
        for(int i=0; i<50; i++) {
            Vec3d pootdir = new Vec3d(0, 0, size);
            pootdir.rotatePitch(randy.nextFloat()*180f);
            pootdir.rotateYaw(randy.nextFloat()*360f);
            ...
            this.worldObj.spawnParticle(EnumParticleTypes.PORTAL, this.posX + pootdir.xCoord, this.posY + pootdir.yCoord, this.posZ + pootdir.zCoord, pootspeed.xCoord, pootspeed.yCoord, pootspeed.zCoord);
        }
        ...
        /**
        List<Entity> sucked = this.worldObj.getEntitiesWithinAABB(null, suckzone, null);
        ...
        */
        ...
        AxisAlignedBB nomzone = new AxisAlignedBB(this.posX - nomrange, this.posY - nomrange, this.posZ - nomrange, this.posX + nomrange, this.posY + nomrange, this.posZ + nomrange);
       /**
        List<Entity> nommed = this.worldObj.getEntitiesWithinAABB(null, nomzone, null);
            ...
            for (int y = -blockrange; y <= blockrange; y++) {
                for (int z = -blockrange; z <= blockrange; z++) {
                    for (int x = -blockrange; x <= blockrange; x++) {
                        ...
                        if (dist <= nomrange && !this.worldObj.isAirBlock(new BlockPos(lx, ly, lz))) {
                            Block b = this.worldObj.getBlockState(new BlockPos(lx, ly, lz)).getBlock();
                            IBlockState b2 = this.worldObj.getBlockState(new BlockPos(lx, ly, lz));
                            float resist = b.getExplosionResistance(this);
                            if (resist <= 10.0) {
                                b.dropBlockAsItemWithChance(worldObj, new BlockPos(lx, ly, lz), b2, 0.9f, 0);
                                this.worldObj.setBlockToAir(new BlockPos(lx, ly, lz));
                            }
                        }
                    }
                }
